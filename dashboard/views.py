...
def sem_temp(request):
    return render(request, 'dashboard/sem_temp.html')
...
def techstack_view(request):
    return render(request, 'dashboard/techstack.html')


def snapview(request):
    return render(request, 'dashboard/snapview.html')  # Adjust the template path as needed
...
def weekly_schedule(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            print("Task added successfully!")  # Debugging log
            return redirect('schedule')  # Redirect to the schedule page
        else:
            print("Form errors:", form.errors)  # Print form errors to debug

    # Fetch tasks grouped by day from the database
    days = ['Sunday', 'Monday', 'Tuesday', 'Wednesday', 'Thursday', 'Friday', 'Saturday']
    schedule = {day: Schedule.objects.filter(day=day).order_by('time') for day in days}

    # Pass the form and schedule data to the template
    form = ScheduleForm()
    return render(request, 'dashboard/schedule.html', {'schedule': schedule, 'form': form})
...
def delete_schedule(request, schedule_id):
    ...
    if request.method == "POST":
        schedule.delete()
        return redirect('schedule')  # Redirect to the schedule page after deletion
...
def assistant_schedule(request):
    if request.method == "GET":
        ...
        schedule_data = {schedule.day: schedule.name for schedule in schedules}
